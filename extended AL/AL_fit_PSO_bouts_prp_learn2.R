...
if ( cluster ){
    ...
    library( doParallel )
    cl = makeCluster( 1, outfile = "output.txt" )
...
source( "AL.do_rapidly_changing_bout_prp_learn_test2.R" )
# source( "AL_new_helper_bout_test.R" )
source( "AL_new_helper_bout_test2.R" )
# Load "expt_info"
...
source( "pso_helper_bouts_prp_learn.R" )

# Load changeover times
load( "min_co_data.RData" )
...
param_bounds = list(
    ...
    transition_prob = c(0,1), prp_learn_prob = c(0,1)
)
...
init_bounds = list(
    ...
    transition_prob = c(0, 0.1), prp_learn_prob = c(0.2,1)
)
####################################
...
for ( i_ in 1:iterations ){
    ...
    costs_ = foreach( n = 1:pop_size ) %dopar% {
        ...
        min_co = minimum_irts[ Subject == subject, V1 ]
        
        # Pretraining
        for ( session_ in 1:15 ) AL.do_fitting( org, pretrain_expt_info, params, min_co )
        # Experiment
        model_event_records = lapply( 1:35, function( session_, org_, params_ ){
            expt_info_ = expt_info[ session_ ]
            AL.do_fitting( org_, expt_info_, params_, min_co )
