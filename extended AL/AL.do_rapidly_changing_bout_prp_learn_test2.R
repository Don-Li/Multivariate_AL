AL.do_fitting = function( org, expt_info_, params, min_changeover ){
    ...
    transition_prob = params[5]
    prp_learn_prob = params[6]
    ...
    time_ = 0
    last_resp = 1; COD_off = 0; resp_value = 1
    rft = F
    ...
    state = 1; start_state = T
    
    repeat{
        # Get the response time
        if ( rft ){
            time_ = time_ + min_prp
            prp_learn( org, resp_value, resp_ind, prp_learn_prob )
            rft = F
            # Emit
            response = emit( org, resp_probs, transition_prob, last_resp, state, start_state )
            ...
            state = response$state
            start_state = response$start_state
            if ( response$resp > 0 & response$resp != last_resp ){
                # Min changeover for S1
                time_ = time_ + min_changeover
            }
        }
